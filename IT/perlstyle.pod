...
perchE<eacute> il punto principale non E<egrave> se l'utente ha scritto B<-> o no.
...
quando il Perl E<egrave> stato installato.
...
per separare le parole. E<Egrave> generalmente piE<ugrave> facile leggere $nomi_come_questi
che $NomiComeQuesti, specialmente per chi non parla il vostro stesso linguaggio.
E<Egrave> anche una semplice regola che E<egrave> coerente con NOMI_COME_QUESTI.
...
Dei buoni messaggi di errore dovrebbero andare su STDERR, includere
...
l'errore standard ritornato dal sistema operativo per capire cosa non
