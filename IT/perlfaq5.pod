...
perlfaq5 - File e Formati ($Revision: 1.16 $, $Date: 2004/05/12 22:44:17 $)
...
Usate il modulo File::Temp, per maggiori informazioni consultate L<File::Temp>.
...
di un normale ps stile Berkeley:
    ...
    print scalar <PS>;
    while (<PS>) {
    ($pid, $tt, $stat, $time, $comando) = unpack($PS_T, $_);
    for $var (qw!pid tt stat time comando!) {
        print "$var: <$$var>\n";
    }
    print 'line=', pack($PS_T, $pid, $tt, $stat, $time, $comando),
        "\n";
    }

Abbiamo usato C<$$var> in una maniera proibita da C<use strict 'refs'>.
Abbiamo cioE<egrave> promosso una stringa a variabile scalare usando i
riferimenti simbolici. Va bene nei piccoli programmi, ma non scala bene.
Inoltre funziona solo con le variabili globali, non con le variabili lessicali.
...
ed un'espressione al posto del filehandle:
    ...
    print  { $fd[1] } "cose carina\n";
    printf { $fd[1] } "Peccato per la povera %x.\n", 3_735_928_559;
    # Peccato per la povera deadbeef.
...
Questa, di Benjamin Goldberg, lo farE<agrave> per voi:

    s/(^[-+]?\d+?(?=(?>(?:\d{3})+)(?!\d))|\G\d{3}(?=\d))/$1,/g;

oppure scritta in maniera prolissa:

    s/(
        ^[-+]?            # l'inizio del numero.
        \d{1,3}?          # la prima cifra prima della prima virgola
...
le ~. Le versioni piE<ugrave> recenti di Perl contengono direttamente questa
	...
	$nomefile =~ s{
	  ...
	  (               # salva in $1
	      [^/]        # un carattere diverso da slash
	            *     # ripetuto una o piE<ugrave> volte (0 significa se stessi)
	  )
	}{
	  $1
	      ? (getpwnam($1))[7]
...
Normalmente perl ignora gli spazi posti alla fine dei nomi dei file, ed
interpreta in maniera speciale alcuni caratteri posti all'inizio (oppure
un "|" posto alla fine).

La versione a tre argomenti di open() vi permette di specificare il modo, 
separatamente dal nome del file. La funzione open() tratta i caratteri
speciali e lo spazio bianco nel nome del file in maniera letterale:

    open FILE, "<", "  file  ";  # il nome del file E<egrave> "   file   "
    open FILE, ">", ">file";     # il nome del file E<egrave> ">file"

PuE<ograve> tuttavia essere molto piE<ugrave> chiaro utilizzare sysopen():
...
(proprietario, ecc.), usate le operazioni di test dei file B<-M>, B<-A> o
...
Va notato che utime() per ora non funziona correttamente con le versioni di
Win95/NT. Un bug E<egrave> stato segnalato. Effettutate dei controlli
accurati prima di usare utime() su queste piattaforme.
...
portabilitE<agrave> come esercizio per i vostri posteri.
    ...
    $FIONREAD = 0x4004667f;         # XXX: dipendente dal OS
