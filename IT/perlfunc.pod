...
formando un valore di lista unico, piE<ugrave> lungo. Gli elementi
di LISTA dovrebbero essere separati da virgole.
...
direttiva E<egrave> in uso, gli operatori C<-x> e <-X> potrebbero restituire il
...
Scrivere C<-exp($foo)> funziona sempre come ci si aspetta che faccia, solo le
...
Va notato che, a differenza di quanto si possa dedurre leggendo C<"Programming Perl"> 
[Programmare Perl, NdT] (il libro del Cammello) o altro, C<:raw> I<non> E<egrave> 
semplicemente l'inverso di C<:crlf>, anche gli altri strati che potrebbero influire 
...
C<:...>, sono chiamate L<strati> di I/O. Per stabilire gli strati di I/O predefiniti, 
...
C<:encoding>, che cambia la condifica predefinita di caratteri per l'handle, consultate 
...
C<\cM\cJ>. CiE<ograve> singnifica che, se non usate binmode() su tali sistemi, le 
...
a due argomenti se la funzione che effettua il bless potrebbe essere ereditta da una classe 
derivata. Consultate L<perltoot> e L<perlobj> per maggiori dettagli sull'operazione di bless 
...
nomi maiuscoli dunque, per prevenire la confusione, potete pure evitare tali nomi di 
...
stampare una traccia dello stack. Il valor di ESPR indica quanti record di attivazione 
...
sicuramente non deve essere una stringa di cifre ottali: C<0644> va bene, C<'0644'> no. 
...
vi potreste aspettare. Similmente, C<chomp $a, $b> E<egrave> interpreato come C<chomp($a), $b> 
...
verrE<agrave> settato a C<0>). Anche chiudere una pipe fa attendere che il processo in 
...
In realtE<agrave>, E<egrave> un'istruzione per il controllo di flusso 
...
Cifra una stringa esattamente come fa la funzione crypt(3) della 
libreria C (assumendo che abbiate a disposizione una versione di 
quest'ultima che non sia stata privata di tale funzione per questioni 
di sicurezza). Questa funzione puE<ograve> risultare utile, tra le 
altre cose, per cercare password poco sicure nei file delle password. 
Solo i ragazzi con il cappello bianco dovrebbero cimentarsi con crypt().

Va notato che C<crypt> E<egrave> intesa come una funzione a senso unico, 
un po' come il rompere le uova per fare la frittata. Non c'E<egrave> 
(tra quelle conosciute) nessuna funzione decrypt (in altre parole, 
crypt() E<egrave> una funzione hash a senso unico). A causa di 
ciE<ograve>, queata funzione non E<egrave> molto utile per la 
crittografia. (Per quest'ultima, consultate il vostro mirror CPAN 
piE<ugrave> vicino).

Quando effetuate la verifica di una stringa cifrata dovete usare il 
testo cifrato come seme (ad esempio C<crypt($chiaro, $cifrato) eq 
$cifrato>). Questo fa sE<igrave> che il vostro codice funzioni sia con 
la C<crypt> standard che con implementazioni piE<ugrave> esotiche. 
In altre parole, non presumete nulla sulla stringa restituita, o su 
quanti byte siano siano significativi nella stringa cifrata.
...
La funzione L<crypt|/crypt> non E<egrave> adatta per cifrare grandi 
quantitE<agrave> di dati, non del tutto almeno, perchE<eacute> non 
potete ottenere le vostre informazioni di nuovo in chiaro. Date 
un'occhiata alle directory F<by-module/Crypt> e a F<by-module/PGP> 
sul vostro mirror CPAN preferito per un mucchio di moduli 
potenzialmente utili.

Se usate C<crypt()> su una stringa Unicode (che I<potenzialmente> 
...
quella copia). Se ciE<ograve> funziona, bene. Altrimenti, C<crypt()> 
...
tentate di settare l'elemento di un hash fittizio all'interno di 
...
nonostante esso abbia trovato "nulla". Ma non E<egrave> che non ha 
...
Restituisce ciascun elemento cancellato oppure 
il valore indefinito se non esiste tale elemento.
Cancellare elementi di C<$ENV{}> causa una modifica
dell'ambiente. Cancellare da una hash legata ad un file
DBM cancella il record dal file stesso. Cancellare 
da un hash o da un array legato non deve necessariamente
restituire qualcosa.
...
dello stesso elemento restituiranno falso. Va notato che 
cancellare elementi nel mezzo di un array
non farE<agrave> scorrere in basso l'indice degli 
elementi successivi, per quello usate splice().
Consultate L</exists>.
...
nuovamente C<die>. Si veda 
L<perlvar/$SIG{expr}> per dettagli sull'utilizzo di C<%SIG>, e
L<"eval BLOCK"> per qualche esempio.  Nonostante questa funzionalitE<agrave>
sia stata pensata per essere eseguita solo appena prima dell'uscita
dal programma, attualmente non E<egrave> cosE<igrave> -- la funzione
...
come prima linea della funzione (vedere L<perlvar/$^S>).  PoichE<eacute>
...
Una forma deprecata di chiamata di subroutine. Consultate L<perlsub>.

=item do ESPR

Utilizza il valore di ESPR come un nome di file ed esegue il contenuto del 
file come fosse uno script Perl. Il suo uso primario E<egrave> quello di 
includere subrotuine da una libreria di subroutine Perl.
...
nome file corrente per i messaggi d'errore, ricerca le librerie in @INC e 
...
di un proramma. Il controllo manuale degli errori puE<ograve> essere fatto in 
questo modo:
...
Quando il nuovo binario viene eseguito, inzierE<agrave> con l'eseguire un
...
poi esegue una C<ungetc>s su di esso, dunque ciE<ograve> non E<egrave> 
...
di C<@ARGV>, e se non avete settato C<@ARGV>, esso leggerE<agrave> 
...
Va notato che, dato che C<eval> cattura gli errori altrimenti blocanti, 
...
C<__DIE__>, quando si usa la forma C<eval{}> come una trappola per le 
eccezioni nelle librerie, si potrebbe non voler innescare alcun hook 
per C<__DIE__> che il codice utente potrebbe aver installato. Per 
...
eccetto che in questa particolare situazione, potete invece semplicemente 
...
PoichE<eacute> E<egrave> un comune errore l'usare C<exec> invece di 
...
No dovete controllare il valore restituito da C<fnctl> con C<defined>.
...
garanzie. Questo significa che dei file sottoposti a lock con 
C<flock> possono essere modificati da quei programmi che non usano 
...
fork(), l'efficienze E<egrave> stata molto curata (per esempio, usando la 
...
Si veda L<perlform> per molti dettagli ed esempi.
...
qualche filehandle, ma potreste anche leggere C<$^A> da 
...
restituita, vengono fornite delle interfaccie per nome nei moduli standard: C<File::stat>, 
C<Net::hostent>, C<Net::netent>,
C<Net::protoent>, C<Net::servent>, C<Time::gmtime>, C<Time::localtime> e C<User::grent>. 
Queste si sovrappongono alle normali funzioni interne, fornendo versioni che restituiscono oggetti 
con i nomi appropriati per ogni campo. Per esempio:
...
Restituisce l'opzione del socket richiesta, o undef se si verifica un errore.
...
Se ESPR viene omessa, globa() utilizza C<$_>. L'operatore C<< <*.c> >> 
...
utilizzato l'estensione standard C<File::Glob>. Consultate 
L<File::Glob> per maggiori dettagli.
...
'struct tm' del C. $sec, $min e $hour sono i secondi, i minuti, e le ora 
dell'orario specificato. $mday E<egrave> il giorno del mese, e $mon E<egrave> 
il mese stesso, nell'intervallo C<0..11>, in cui 0 indica Gennaio e 11 Dicembre. 
$year E<egrave> il numero di anni a partire dal 1900. CiE<ograve> significa che 
$year E<egrave> 123 nel 2023. $wday E<egrave> il giorno della settimana, con 0 
che indica Domenica e 3 MercoledE<igrave>. $yday E<egrave> il giorno dell'anno, 
nell'intervallo C<0..364> (o <0..365> negli anni bisestili).

Va notato che l'elemento $year I<non> contiene semplicemente le ultime due cifre 
dell'anno. Se assumete ciE<ograve>, allora vi ritrovate a creare programmi non 
compatibili con l'anno 2000 (Y2K), e non volete fare ciE<ograve>, vero?

La via corretta per ottenere un anno a 4 cifre E<egrave> semplicemente:

	$year += 1900;

E per ottenere le ultime due cifre dell'anno (es. '01' nel 2001) potete scrivere:
...
In contesto scalare, C<gmtime()> restituisce il valore di ctime(3):
    ...
    $stringa_adesso = gmtime;  # es. "Thu Oct 13 04:54:34 1994"

Consultate anche la funzione C<timegm> fornita dal modulo C<Time::Local>, 
e la funzione strftime(4) disponibile attraverso il modulo POSIX.

Questo valore scalare B<non> dipende dal locale (consultate L<perllocale>), 
ma E<egrave> interno a Perl. Consultate anche il modulo C<Time::Local> 
(per riconvertire secondi, minuti, ore, ... nel numero di secondi trascorsi 
dallo scoccare della mezzanotte del 1o Gennaio 1970, il valore restituito 
da time()), e le funzioni strftime(3) e mktime(3) disponibili 
attraverso il modulo POSIX. Per ottenere stringhe di data simili, ma 
dipendenti dal locale, impostate le vostre varaibili d'ambiente relative 
al locale in maniera appropriata (consultate L<perllocale>) e provate ad 
esempio:
...
necessariamente lunghi tre caratteri in tutti i locale.
...
corrispondente. (Per convertire stringhe che iniziano con 0, 0x, o 0b, 
...
=item index STR,SUBSTR,POSIZIONE
...
una ricerca tramite regular expression. Essa restituisce la posizione 
della prima occorrenza di SUBSTR in STR, a partire da POSIZIONE. Se 
POSIZIONE viene omessa, index() inizia a cercare dall'inizio della stringa. 
Il valore restituito indica la posizione a partire da C<0> (o qualsiasi 
valore a cui abbiate impostato la variabile C<$[>--ma non fatelo). Se la 
sottostringa non viene trovato, index() restituisce un'unitE<agrave> meno 
della base, cioE<egrave> solitamente -1.
...
per ottenere le corrette definizioni delle funzioni. Se F<ioctl.ph>
...
La stringa speciale "C<0> ma vera" non risente delle proteste
...
(In contesto scalare, restituisce il numero di chiavi). Le chiavi sono 
restituite in ordine apparentemente casuale. Tale ordine casuale 
...
Come effetto collaterale, la funzione reimposta l'iteratore dell'HASH.
...
L<perlipc/"Sockets: Client/Server Communication"> ["Comunicazione Client/Server", NdT].
...
Tutti gli elementi della lista sono numerici, e derivano direttamente dalla 
'struct tm' del C. $sec, $min e $hour sono i secondi, i minuti, e le ore 
dell'orario specificato. $mday E<egrave> il giorno del mese, e $mon E<egrave> 
il mese stesso, nell'intervallo C<0..11>, in cui 0 indica Gennaio e 11 Dicembre. 
$year E<egrave> il numero di anni a partire dal 1900. CiE<ograve> significa che 
$year E<egrave> 123 nel 2023. $wday E<egrave> il giorno della settimana, con 0 
che indica Domenica e 3 MercoledE<igrave>. $yday E<egrave> il giorno dell'anno, 
nell'intervallo C<0..364> (o <0..365> negli anni bisestili). $isdst assume valore 
vero se l'orario specificato cade durante l'ora legale, e valore falso altrimenti.
...
In contesto scalare, C<localtime()> restituisce il valore di ctime(3):
    ...
    $stringa_adesso = localtime;  # es. "Thu Oct 13 04:54:34 1994"
...
Fa le stesse cose della funzione C<stat> (compreso il settare lo speciale 
...
PoichE<aacute> perl non guarda avanti alla ricerca della
...
dal reale messaggio. Questo impacchettament puE<ograve> venire aperto con 
...
coda di messaggi ID. MSG deve inziare con un intero long nativo che indica il tipo di 
...
Si veda la funzione L</use>, della quale C<no> E<egrave> l'opposto.
...
una stringa esadecimale. Se ESPR inizia con X<0x>, viene interpretata come 
una stringa binaria. Qualsiasi spazio bianco che faccia da prefisso viene 
ingnorato in tuti e tre i casi). Il seguente codice E<egrave> in grado di 
...
(Quella che segue E<egrave>egue una documentazione completa di open(): 
...
il suo output a noi. Consultate L<perlipc/"Using open() for IPC"> per altri 
esempi su questo caso. (Non E<egrave> ammesso l'uso di C<open> per eseguire un 
comando e accedervi in scrittura e lettura, consultate L<IPC::Open2>, 
...
input e output vengono processatti (consultate L<open> e L<PerlIO> per 
...
nel caso di tre parametri, gli strati impostati di default da C<open> vengono 
ignorati).
...
Un caso speciale, la versione a tre argomenti in modalitE<agrave> 
...
I filehandle possono essere aperti in file "in memoria" contenuti in 
scalari Perl, mediante:
...
"in memoria", prima dovete chiuderli:
    ...
    close STDOUT;
    open STDOUT, '>', \$variable or die "Impossibile aprire STDOUT: $!";
...
Esempio:
    ...
    open(EXTRACT, "|sort >/tmp/Tmp$$")		# $$ e` il pid del nostro processo
...
per essere duplicato e aperto. Potete usare C<&> dopo C<< > >>, C<<< >> >>>, 
...
di IO). Se usate la versione a tre argomenti, potete passare un numero, il nome di 
...
Per esempio:
    ...
    # aopertura in input, riutilizzando il fileno di $fd
    open(FILEHANDLE, "<&=$df")
...
l'uso di lock tramite flock(). Se semplicemente utilizzate 
...
piattaforma. Una buona regola E<egrave>: se il tuo sistema operativo 
ha una vera C<fork()> (in altre parole, se E<egrave> UNIX) puoi usare la 
list form.
...
L<perlport>). Per essere sicuri, dovreste settare C<$|> ($AUTOFLUSH nel modulo 
...
il flag sarE<agrave> settato per ogni nuovo descrittore di file aperto, come 
...
F<"rsh cat file |">, oppure voi potreste cambiare alcuni particolari nomifle 
...
quali sono mappati alla fopen() del C). Questa E<egrave> un'altra possibilita per 
...
C<our> dichiara le variabili indicate come globali valide, nel blocco 
che le include, file, o C<eval>. CiE<ograve> significa che le regole 
di scoping sono le stesse della dichiarazione "my", ma non viene 
creata una variabile locale. Se E<egrave> indicato piE<ugrave> di un 
valore, la lista deve essere inclusa tra parentesi. La dichiarazione 
C<our> non ha alcun effetto semantico a meno che non sia attivo 
"use strict vars", nel qual caso essa consente di utilizzare la 
variabile globale dichiarata senza qualificarla con un nome di package. 
(Ma solo all'interno dello scope lessicale della dichiarazione C<our>. 
In questo essa differisce da "use vars", il cui scope E<egrave> a 
livello di package).
...
Sono permesse multiple dichiarazioni C<our> all'interno dello stesso 
scope lessicale, se si trovano all'interno di differenti package. Se 
si trovano nello stesso package, Perl genererE<agrave> degli 
avvertimenti solo se li avete richiesti.
...
ed il tipo dei valori, come indicato di seguito:
    ...
    w	Un intero compresso BER. I suoi byte rappresentano un
...
I tipi C<a>, C<A>, and C<Z> prelevano solamente un valore, ma lo 
...
significativo del byte di input corrispondente, ad esempio con 
C<ord($byte)%2>. In particolare i byte C<"0"> e C<"1"> generato i bit 
0 e 1, e cosE<igrave> fanno anche i byte C<"\0"> e C<"\1">.
...
significativi del byte in input, come ad esempio in C<ord($byte)%16>. 
...
corrispondentente valore per C<p> o C<P> E<egrave> C<undef>. 
Un comportamento simile si ha con unpack().

=item *

Il template C</userdefined>  permette di compattare e scompattare 
stringhe in cui la struttura compattata contiene un conteggio di 
byte seguito dalla stringa stessa. Dovete scrivere 
I<lunghezza-elemento>C</userdefined>>I<stringa-elemento>.
...
La I<stringa-elemento> deve, al momento, essere C<"A*">, C<"a*"> 
...
C<a> o C<Z> puE<ograve> causare l'inserimento di catteri C<"\000">, 
...
essi sono identici a C<i> and C<I>.
...
Consultate L<perlmod/"Package"> per maggiori informazioni sui 
...
potreste aver bisogno di settare C<$|> per svuotare il vostro 
...
quale l'ultima ricerca C<m//g> ha trovato qualcosa (se la varaibile 
non E<egrave> specificata, viene usata C<$_>. Il valore che determina 
la posizione puE<ograve> essere modificato. Tale modifica 
influenzerE<agrave> l'asserzione (lunga zero caratteri) C<\G> nelle 
espressioni regolari. 

Consultate L<perlre> e L<perlop>.
...
con la corrispondente parentesi di chisura, inserite un C<+> o 
mettere parentesi attorno a tutti gli argomenti.

Va notato che, se i FILEHANDLE che utilizzate sono contenuti 
in un array o in un'altra espressione, dovrete utilizzare un 
blocco che ne restituisca il valore:
...
ma E<egrave> piE<ugrave> efficiente. Restituisce il nuovo numero di 
elementi nell'array.
...
precedeuti da un backslash. (Il che signfica che tutti i caratteri 
...
settaggio del locale). Questa E<egrave> la funzione interna che 
...
se si E<egrave> verificato un errore. La lunghezza di SCALARE 
sarE<agrave> aumentata o diminuita della lunghezza di caratteri letta.
...
I/O C<:utf8> (si veda L</open>, o la direttiva C<open>, L<open>), le operazioni di 
...
in dettaglio in L<perlop/"I/O Operators"> ["Operatori I/O", NdT].

    $line = <STDIN>;
    $line = readline(*STDIN);		# stessa cosa

Se readline incontra un errore del sistema operativo, $! verrE<agrave> 
...
al ciclo piE<ugrave> interno che lo include. Questo comando viene usato 
di norma da quei programmi che voglio ingannare loro stessi a proposito 
dell'input:
...
di sistem I<mv> a volte compensa questa carenza. Altre restrizioni includono 
...
"F<Foo/Bar.pm>" nelle directory specificate nell'array C<@INC>.
...
La soubroutine deve restituire C<undef> oppure un filehandle, dal quale 
...
dei plugin di <programma con plugin> E<egrave> permesso grazie a degli hook.

(***) letteralmente santificato, consacrato, si intende di un oggetto che 
E<egrave> stato legato al nome di un package. Si veda la funzione bless.
...
=item rindex STR,SUBSTR,POSIZIONE
...
non cambia la posizione corrente, I<ma> fa in modo che la condizione di fine 
...
compattazioni di directiry, ha gli stessi avvertimenti della corrispondente 
routine della libreria di sistema.
...
SIGALRM) dipende dall'implementazione.
...
per ottenere le definzioni di costante corrette. Se CMD E<egrave> IPC_STAT 
o GETALL, allora ARG deve essere una variabile, che conserverE<agrave> la struttura 
C<semid_ds> restituita. Restituisce dei valori come fa C<ioctl>: il valore 
...
Consultate anche la documentazione di 
L<perlipc/"SysV IPC">, C<IPC::SysV> e C<IPC::Semaphore>.
...
C<pack("s!3", $numsem, $opsem, $flagsem)>. Il numero di operazioni sui semafori 
E<egrave> implicitamente fornito dalla lunghezza di STRINGAOPZ. Restituisce vero in 
caso di successo, oppure falso nel caso si verifichi un errore. Ad esempio, il 
seguente codice attende sul semaforo $numsem dell'id di semaforo $idsem:
...
L<perlipc/"UDP: Message Passing"> ["UPD: Trasferimento di messaggi", NdT] 
per degli esempi.
...
pmessi, li imposta di default a C<0,0>. Va notato che la versione BSD 4.2 
...
indefinito se si verifica un errore. VALOREOPZ puE<ograve> 
essere specificato come C<undef> se non desiderate passare un 
argomento.
...
per ottenere le definzioni di costante corrette. Se CMD E<egrave> 
...
troppo corta, dei null verrano scritti per rimepire un numero 
...
anche una forma di chiusura piE<ugrave> drastica di C<close> 
...
nel quale una chiamama a C<pipe(Rdr, Wtr)> E<egrave> essenzialmente:
...
passati per riferimento, quindi non modificateli.
...
elementi in input che risultano uguali. BenchE<egrave>
...
non lo farE<agrave>. Consultate L</use>.
...
NaN (not-a-number) [non-un-numero, NdT], e poichE<eacute> sort genererE<agrave> 
...
Divide una stringa in una lista di stringhe e restituisce
tale lista. Di default, i campi vuoti iniziali sono 
mantenuti, mentre quelli in coda vengono cancellati.
...
Il pattern vuoto C<//> corrisponde specificamente 
alla stringa vuota, e non deve essere confuso con l'uso
di C<//> per indicare "l'ultimo pattern soddisfatto con
successo".
...
Il flag "vettore" C<v>, opzionalmente la stringa da utilizzare per unire i 
valori. Questo flag dice al perl di interpretare la stringa fornita come un vettore di 
interi, uno per ogni carattere nella stringa, separati da una stringa (un punto C<.> per 
default). PuE<ograve> essere utile per stampare i valori ordinali dei caratteri in una 
stringa arbitraria:
...
Lo scopo della funzione E<egrave> seminare ["seed" in iglese, NdT] 
...
Va notato che vi serve qualcosa di piE<ugrave> casuale del
seme di default per usi inerenti la crittografia. 
...
(*) Il campo ctime non E<egrave> portabile, in particolare non ci si puE<ograve> aspettare sia 
...
stat provenienti dall'ultima stat o filestat. Ad esempio:
...
Potete scrivere le ultime due utilizzando gli operatori C<-u> e C<-d>. Le costanti S_IF* 
...
Per ottenere le informazioni di stato per un link simbolico invece che del file obiettivo 
che sta dietro al link, usate la funzione C<lstat>, consultate L</stat>.
...
Cercando C</\bfoo\b>, verranno controllati solo le posizioni in C<$_> che
contengono C<f>, poichE<eacute> essa E<egrave> piE<ugrave> rara di C<o>. 
...
Notate che se dovete cercare stringhe che non conoscerete se non quando
...
Consultate L<perlsub> e L<perlref> per dettagli sulle subroutine e i riferimenti, L<attributi> 
...
Se il valore assegnabile restituito da substr viene usato dopo che la ESPR E<egrave> stata 
cambiata in un qualche modo, il comportamento potrebbe non essere quello atteso ed E<egrave> 
soggetto a cambiamenti. Questo avvertimento comprende codice come 
C<print(substr($pippo,$a,$b)=$pluto)> oppure C<(substr($pippo,$a,$b)=$pluto)=$paperino> (dove 
$pippo viene cambiato tramite l'assegnazione della sottostringa, e poi subst viene utilizzato di 
nuovo), o dove viene creato un alias di una substr() tramite un ciclo C<foreach>, oppure viene 
passata come parametro, oppure viene preso un riferimento ad essa e poi l'alias, il parametro, 
oppure il riferimento dereferenziato viene utilizzato dopo che l'ESPR originale E<egrave> stato 
cambiato, oppure viene assegnato e poi utilizzato un seconda volta.
...
Invoca la chiamata di sistema specificata come primo elemento della lsta, passando i rimanenti 
...
invocato. Se la chiamata di sistema fallice, C<syscall> restituisce C<-1> ed imposta C<$!> 
(errno). Va notato che alcune chiamate di sistema possono legittimamente restituire C<-1>. 
La maniera approrpiata per gestire tali chiamate E<egrave> quella di assegnare C<$!=0;> 
...
esclusiva. Questo C<non> E<egrave> eseguire un lock: l'esclusivitE<agrave> significa in questo 
contesto che se il file esiste giE<agrave>, sysopen() fallisce. La C<O_EXCL> vince C<O_TRUNC>.

Talvolta potreste voler troncare un file giE<agrave> esistente: C<O_TRUNC>.

C<0644> come argomento di sysopen andrebbe usato raramente, se non mai, poichE<eacute> toglie 
...
oppure undef se c'E<egrave> stato un errore. SCALARE verrE<agrave> aumentato o diminuito in 
maniera che l'ultimo byte effettivamente letto sia l'ultimo byte dello scalare dopo la lettura.
...
strato C<:utf8>. Consultate L</binmode>, L</open> e la direttiva di open, L<open>.
...
C<sysread>, per esempio &gt;&lt oppure read()), C<print>, C<write>, C<seek>, C<tell> o C<eof>, 
...
posizione pari a zero viene restituito sotto forma della stringa C<"0 ma vera">; dunque 
...
pari a -1 indica un insuccesso nel far eseguire il programma (esaminate $!, per il motivo).
...
programma.
    ...
    system(@args) == 0
	 or die "system @args E<egrave> fallito: $?"
...
SCALARE al FIELHANDLE specificato, usando la chiamata di sistema write(2). Se LUNGHEZZA 
...
c'E<egrave> stato un errore. Se la LUNGHEZZA E<egrave> piE<ugrave> grande dei dati 
disponibili nello scalare dopo lo SCOSTAMENTO, verrE<agrave> scritto solo il maggior 
...
Consultate L</binmode>, L</open> e la direttiva di open, L<open>.
...
Non usate tell() su un filehandle che E<egrave> stato aperto usando sysopen(), per 
questo, usate sysseek() come descritto sopra. PerchE<eacute>? PerchE<eacute> sysopen() 
crea dei filehandle privi di buffer, "raw" [grezzo, NdT], mentre open() crea dei 
filehandle con buffer. sysseek() ha senso solo per il primo tipo, tell() ha senso solo 
per il secondo tipo.
...
locazione in una directory. Ha gli stessi avvertimenti sulla directory compaction 
[compattazione della directory, NdT] della corrispondente routine della libreria di 
sistema.
...
Restituisce il numero di secondi, senza tenere conto dei I<leap second> 
[secondi di aggiustamento, NdT], dalla data che il sistema considera essere l'epoch 
(cioE<egrave> 00:00:00, 1 Gennaio, 1904 su MacOS, e 00:00:00 UTC, Gennaio 1, 1970, sulla 
maggior parte degli altri sistemi). Tale numero E<egrave> utilizzabile come parametro a 
C<gmtime> e C<localtime>.
...
secondi, per il processo corrente e per i figli di questo processo.
    ...
    ($utente,$sistema,$figlioutente,$figliosiatema) = times;
...
Setta l'umask per il processo a ESPR e restituisce il precedente valore.
...
scalare, un array (utilizzando C<@>), un hash (utilizzando C<%>), una subrotuine 
(utilizzando C<&>), oppure un typeglob (utilizzando <*>). (C<undef $hash{$chiave}> 
...
Nota: C<unlink> non cancellerE<agrave> le directory a meno che non siate superuser e non 
venga bassato il flag B<-U> al Perl. Anche se queste condizioni risultano soddisfatte, 
sappiate che effettuare un unlink su una directory poE<egrave> danneggiare seriamente il 
vostro filesystem. Usate C<rmdir> al suo posto.
...
Siccome quest'interfaccia E<egrave> molto aperta, i pragma (direttive del compilatore) 
...
importate da C<use>, cioE<egrave> chiama C<unimport Module LIST> anzichE<eacute> 
C<import>.
...
ha lo stesso effetto del comando Unix touch(1) se il file I<esiste giE<agrave>>.
...
del file all'istante corrente (equivalente cioE<egrave> al suddetto esempio).
...
Va notato che solo passare uno dei primi due elementi come undef risulterE<agrave> equivalente 
...
essere lo stesso ordine che producono le funzioni C<values> e C<each> sullo stesso 
...
poi E<egrave> possibile eseguire un wait non bloccante per tutti i processi zombi 
...
Restituisce vero se il contesto della subroutine attualmente in esecuzione si aspetta un 
valore di lista. Restituisce falso se il contesto si aspetta uno scalare.
...
un precedente eval), tale valore viene utilizzato dopo aver aggiunto C<"\t...caught"> a 
C<$@>. Questo E<egrave> utile per rimanere quasi, ma non del tutto, uguali a C<die>.
...
ritene opportuna (ad esempio, convertendolo in una C<die>). La maggior parte degli 
...
variabile C<$->, la quale puE<ograve> essere settata a C<0> per forzare una nuova pagina.
