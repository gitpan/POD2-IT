...
perlfaq9 - Programmazione di rete ($Revision: 1.14 $, $Date: 2004/05/22 01:11:41 $)
...
La specifica CGI originale si trova su: http://hoohoo.ncsa.uiuc.edu/cgi/

La bozza RFC corrente sulle migliori pratiche di utilizzo si trova su:
http://CGI-Spec.Golux.Com/

Altri documenti rilevanti sono elencati qui:
...
Usate un modulo standard, solitamente CGI.pm. Non dovete tentare di farlo
a mano in nessuna circostanza!

Avrete a che fare con un sacco di programmi CGI che ciecamente, per i POST,
leggono da STDIN un numero di byte pari a CONTENT_LENGTH, o prendono la
stringa QUERY_STRING per decodificare i GET. Questi programmi sono scritti
molto male. Funzionano soltanto a volte. Tipicamente dimenticano di
controllare il valore di ritorno della chiamata di sistema read(), che
E<egrave> un peccato capitale. Non gestiscono le richieste HEAD. Non
gestiscono i moduli multipart usati per il caricamento dei file. Non
gestiscono le combinazioni GET/POST in cui i campi della richiesta stanno
in piE<ugrave> di un posto. Non gestiscono le parole chiave nella stringa
che contiene la richiesta.

In breve, sono fatti coi piedi. Resistete ad ogni costo. Non fatevi tentare
dal reinventare la ruota. Usate CGI.pm o CGI_Lite.pm, invece (disponibili
su CPAN); oppure, se siete intrappolati nelle lande senza moduli di
perl1 .. perl4, date un'occhiata a cgi-lib.pl (disponibile a questo
indirizzo: http://cgi-lib.stanford.edu/cgi-lib/ ).

Assicuratevi di sapere quando usare GET o POST nella vostra form. GET
dovrebbe essere usata soltanto quando lo stato del server non viene
modificato. In caso contrario potreste ritrovarvi con database manomessi
e messaggi di feedback ripetuti. Una bella parola per indicare tutto
questo E<egrave> "idempotenza". Vuole semplicemente dire che non
dovrebbe esserci alcuna differenza nel fare una sola volta una richiesta
GET per una certa URL, o nel farla piE<ugrave> volte. Questo perchE<eacute>
la definizione del protocollo HTTP dice che una richiesta GET puE<ograve>
essere messa in cache dal browser, dal server o da un proxy. Le richieste
di tipo POST non posso essere messe in cache, poichE<eacute> ogni richiesta
E<egrave> indipendente e conta come unica. Tipicamente, le richieste di
tipo POST dipendono dallo stato del server (fare una richiesta ad un database
o aggiornarlo, inviare una mail, comprare un computer). 
...
E<egrave> eccessivo. Ecco un programma che ordina i messaggi.
    ...
    #!/usr/bin/perl
    # bysub1 - semplice ordinamento secondo il soggetto
    my(@msgs, @sub);
    my $msgno = -1;
...
Di solito, la maniera per sapere il vostro hostname E<egrave> chiamare il
programma C<`hostname`>. BenchE<eacute> rapido, dE<agrave> luogo a qualche
problema, come il non sapere se si abbia o meno ricevuto il nome canonico. 
E<Egrave> uno di quei compromessi tra convenienza e portabilitE<agrave>.

Il modulo Sys::Hostname (parte della distribuzione standard) vi darE<agrave>
l'hostname attraverso il quale potreste ottenere l'indirizzo IP (posto che
abbiate un DNS funzionante) con una chiamata a gethostbyname().

     use Socket;
     use Sys::Hostname;
     my $host = hostname();
     my $indi = inet_ntoa(scalar gethostbyname($host || 'localhost'));

Probabilmente il modo piE<ugrave> semplice per conoscere il vostro nome a
dominio E<egrave> dedurlo da /etc/resolv.conf, almeno in ambiente Unix.
Naturalmente, questo implica diverse assunzioni a proposito della vostra
configurazione di resolv.conf. Ad esempio, che esista.

(Non abbiamo ancora a dispozione un buon sistema di riconoscimento dei nomi
a dominio per i sistemi non unix)
...
Un modulo chiamato DCE::RPC E<egrave> in fase di sviluppo (benchE<eacute>
ancora non disponibile) e sarE<agrave> rilasciato come parte del pacchetto
DCE-Perl (disponibile su CPAN). La suite rpcgen, disponibile presso
CPAN/authors/id/JAKE/, E<egrave> un generatore di stub RPC e include il
modulo RPC::ONC.
