...
perlfaq6 - Espressioni Regolari ($Revision: 1.10 $, $Date: 2004/05/20 00:43:57 $)
...
Qui c'e il codice che trova le frasi che iniziano con "From " ["da", NdT]
...
Usate la forma a quattro argomenti di sysread per aggiungere ininterrottamente
...
Quello che segue E<egrave> estremamente inefficiente:

     # una maniera ovvia ma lenta e`:
     @capcitta = qw(CO PA MI NA MN);
     while (defined($linea = <>)) {
	for $citta (@capcitta) {
	    if ($linea =~ /$citta/i) {
		print $linea;
...
Questo perchE<eacute> il Perl deve ricompilare tutti questi pattern per ogni
linea del file. Dalla versione 5.005, esiste un approccio decisamente
migliore, che fa uso del nuovo operatore C<qr//> :

     # usa il nuovo ed elegante operatore qr// , anche con il flag /i
     use 5.005;
     @capcitta = qw(CO PA MI NA MN);
     @pattern   = map { qr/$_/i } @capcitta;
     while (defined($linea = <>)) {
	for $oggettopatt (@pattern) {
	    print $linea if $linea =~ /$oggettopatt/;
	}
     }

=head2 PerchE<eacute> non mi funzionano le ricerche del limite di una parola effettuate con \b?

Due comuni malintesi sono che C<\b> sia un sinonimo di C<\s+> e che sia
il punto di congiunzione tra caratteri spazio e caratteri non spazio.
Entrambe le supposizioni sono scorrette. C<\b> E<egrave> il luogo tra un
carattere C<\w> ed un carattere C<\W> (vale a dire, C<\b> E<egrave> il
limite di una "parola"). Si tratta di un'asserzione di ampiezza zero, proprio
come C<^>, C<$> e tutte le altre anchor, dunque non consuma alcun carattere.
L<perlre> descrive il comportamento di tutti i metacaratteri delle
espressioni regolari.

Qui ci sono degli esempi di applicazione scorretta di C<\b>, con le
relative correzioni:

    "due parole" =~ /(\w+)\b(\w+)/;           # SBAGLIATO
    "due parole" =~ /(\w+)\s+(\w+)/;          # giusto
    " =senzamatch= testo" =~ /\b=(\w+)=\b/;   # SBAGLIATO
    " =senzamatch= testo" =~ /=(\w+)=/;       # giusto

Sebbene possano non fare quel che pensate, C<\b> e C<\B> possono ugualmente
essere piuttosto utili. Per un esempio dell'uso corretto di C<\b>, date
un'occhiata all'esempio sul match delle parole duplicate su piE<ugrave> linee.

Un esempio di uso di C<\B> E<egrave> il pattern C<\Bsono\B>. Esso
troverE<agrave> le occorrenze di "sono" solo all'interno delle parole,
come in "tassonomia" ma non "consono" o "sonoro".

=head2 PerchE<eacute> utilizzare $&, $` o $' rallenta il mio programma?

Quando Perl si accorge che il vostro programma utilizza una di tali
variabili, le fornisce in ogni operazione di 'pattern matching'. Il
meccanismo che gestisce queste variabili E<egrave> lo stesso che fornisce
il supporto per l'uso di $1, $2 etc. Quindi, pagate lo stesso prezzo per
ogni regex che contiene delle parentesi che catturano. Se non utilizzate
$&, $` e $' in nessun punto del programma, allora le regex I<senza>
parentesi che catturano non pagano pegno.

Concludendo, se potete evitate di usare $&, $' e $`, ma se proprio dovete
usarle allora usatele quanto volete, visto che il prezzo E<egrave> lo
stesso e lo avete giE<agrave> pagato. Tenete presente che alcuni algoritmi
apprezzano molto l'uso di queste variabili. A partire dalla release 5.005,
...
non vi garba l'utilizzo di (?!>), potete sostituire (?!<[A-Z]) con
(?:^|[^A-Z]).
