...
affinchE<egrave>�l'asserzione (statement) sia vera:

    "Ciao Mondo" =~ /mondo/;  # non matcha, case sensitive
    "Ciao Mondo" =~ /o M/;    # matcha, ' ' E<egrave> un carattere qualsiasi
    "Ciao Mondo" =~ /Mondo /; # non matcha, non c'E<egrave> un ' ' alla fine

perl effettuerE<agrave> sempre un match nel punto piE<ugrave> remoto/lontano della stringa:

    "Ciao Mondo" =~ /o/;        # matcha 'o' in 'Mondo'
...
Un metacarattere puE<ograve> essere matchato inserendo un backslash prima di lui:
    ...
    "2+2=4" =~ /2+2/;    # non matcha, + E<egrave> un metacarattere
    "2+2=4" =~ /2\+2/;   # matcha, \+ E<egrave> trattato come un semplice +
    'C:\WIN32' =~ /C:\\WIN/;                       # matcha
    "/usr/bin/perl" =~ /\/usr\/local\/bin\/perl/;  # matcha

Nell'ultima regex, le successive slash C<'/'> (N.di T. rispetto alla prima)
...
ad es., C<\x1B>:
    ...
    "cat"        =~ /\143\x61\x74/ # matcha, ma E<egrave> un modo bizzarro per pronunciare gatto
...
E<egrave> C<'a'>.
    ...
    /[yY][eE][sS]/; # matcha 'yes' in modalitE<agrave> case-insensitive
                    # 'yes', 'Yes', 'YES', etc.
    /yes/i;         # anche questo matcha 'yes' in modalitE<agrave> case-insensitive way
...
carattere, o il match fallisce.  Dunque
    ...
    /[a^]at/;  # matcha 'aat' o '^at'; qui '^' E<egrave> normale
...
alla prossima posizione nella stringa.  Alcuni esempi:
    ...
    "cats and dogs" =~ /cat|dog|bird/;  # matcha "cat"
...
sono
    ...
    /house(cat(s|)|)/;  # matcha sia 'housecats' sia 'housecat' sia
                        # 'house'.  Nota che i raggruppamenti possono essere annidati.

    "20" =~ /(19|20|)\d\d/;  # matcha l'alternativa nulla '()\d\d',
                             # perchE<egrave> '20\d\d' non puE<ograve> matchare
...
B<backreferences> C<\1>, C<\2>, ...  Le backreferences sono
...
Qui ci sono alcuni esempi:
    ...
    /(\w+)\s+\1/;    # matcha parole doppie separate da uno o piE<ugrave> spazi di lunghezza 
	 					   # arbitraria
    $year =~ /\d{2,4}/;  # si assicura che l'anno sia almeno di 2 cifre ma non piE<ugrave>
                         # di 4 cifre
...
stampa
    ...
    Word is dog, finisce alla posizione 7
    Word is house, finisce alla posizione 13
...
Il modificatore di valutazione C<s///e> wrappa un C<eval{...}> attorno alla stringa
...
C<s'''>, allora la regex e la sostituzione sono trattate come single quoted strings.
...
per i quali la C<stringa> �divisa.  Per esempio, per dividere una stringa in parole, 
si usi
