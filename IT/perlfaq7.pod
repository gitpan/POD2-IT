...
perlfaq7 - Questioni Generali Sul Linguaggio Perl ($Revision: 1.12 $, $Date: 2004/04/12 00:18:15 $)
...
Notate che <FILE> non E<egrave> uno specificatore di tipo per i file e nemmeno
...
Normalmente, una parola non posta tra virgolette puE<ograve> rimanere tale, ma
...
Se avete una versione piE<ugrave> vecchia Perl, la variabile '$^W'
...
E<eacute> da notare che, come per tutte le variabili speciali [quelle
...
Un modulo E<egrave> un package che risiede in un file che ha lo stesso nome.
Per esempio, il modulo Eccoci::Qua risiede in Eccoci/Qua.pm . Per avere
maggiori dettagli leggete L<perlmod>. Troverete utile anche L<Exporter>.
Se state scrivendo un modulo in C o uno misto di C e Perl,
allora dovreste studiare L<perlxstut>.

C<h2xs> E<egrave> un programma che crea per voi una traccia di tutto il
necessario.

   % h2xs -XA -n Mio::Modulo

Il parametro C<-X> indica a C<h2xs> che non si sta usando codice d'estensione
C<XS>. Il parametro C<-A> indica a C<h2xs> che non si sta usando AutoLoader 
e il parametro C<-n> specifica il nome del modulo. Consultate L<h2xs> per
maggiori dettagli.
...
perde il valore di una variabile. E<eacute> causato da questioni di
...
si compie il ciclo). Ad ogni modo non E<egrave> cosE<igrave>. Questo era un 
errore, ora risolto nelle ultime versioni (testato con 5.004_05, 5.005_03 e
5.005_56). 
...
L<perldata/"Typeglobs and Filehandles"> e in modo particolare L<perlsub/"Pass by Reference">.
...
Notate che C<qr//> permette di usare i flag alla fine. Quel pattern viene
...
Come per molte cose che riguardano il Perl, c'E<egrave> piE<ugrave> di un modo
per farlo. CiE<ograve> che in altri linguaggi E<egrave> una "variabile statica"
in Perl potrebbe essere una variabile privata ad una funzione
(una variabile che E<egrave> visibile in una sola funzione, e mantiene
il suo valore tra una chiamata e l'altra), oppure una variabile
privata ad un file (visibile soltanto alle funzioni contenute nel
file in cui essa E<egrave> stata dichiarata).
...
Ora contatore_precedente() e contatore_successivo() condividono una variabile privata $contatore,
che E<egrave> stata inizializzata durante la compilazione.

Per dichiarare una variabile privata ad un file, farete ancora uso di my(),
ponendo la dichiarazione all'inizio del file, nel blocco piE<ugrave> esterno.
Ponete che questo sia il file Pax.pm:
...
Quando C<use Pax> oppure C<require Pax> caricano questo modulo, la variabile
sarE<agrave> inizializzata. Non sarE<agrave> distrutta dal garbage collector
come avviene per la maggior parte delle variabili che non sono piE<ugrave>
visibili, perchE<egrave> la funzione cominciato() se ne occupa, ma nessun
altro puE<ograve> ispezionarne il contenuto. Non si chiama $Pax::inizio
perchE<egrave> la sua visibilitE<agrave> non E<egrave> legata al package.
La sua visibilitE<agrave> coincide con il file. Potenzialmente potreste
avere diversi package nello stesso file che accedono alla stessa variabile,
ma un altro file con lo stesso package non potrebbe accederle.
...
Si noti come il valore "privata" non viene mai scritto. CiE<ograve>
...
per trovare molti altri esempi di questo stile.
...
Potete usare il POD incluso nel codice per mettere quest'ultimo da parte.
La direttiva =for dura fino al paragrafo che segue (due ritorni a capo consecutivi).
...
E usare solo un hash a piE<ugrave> livelli.
...
Se vedete "If you see "bad interpreter - no such file or directory" ["interprete 
...
piuttosto che /usr/local/bin/perl .
...
Copyright (c) 1997-2002 Tom Christiansen e Nathan
