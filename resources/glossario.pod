...
Non c'e` una reale traduzione (credo), quindi va utilizzata una frase
intera...
...
Linkare e` usato nella parlata corrente. Non mi e` noto un verbo
italiano che si possa usare al posto del neologismo.

(arthas) secondo me la traduzione corretta e` "collegare"
...
Pattern matching e` la ricerca di un pattern (vedi voce, sopra),
match a pattern e` trovare un pattern secondo me [Arthas]
...
Secondo me pattern e` meglio lasciarlo cosi` (o tradurlo con schema, a
seconda del contesto), matching in questo caso puo` diventare ricerca, ma
guardate I<to match> piu` sotto per delucidazioni ulteriori.
...
Non si e` trovato niente di meglio che una perifrasi per tradurre
...
limite e` meglio specificare tra parentesi qualche dettaglio. Vedete
...
Ancora piu` complesso e` il caso di untaint. Se per 'untainted'
forse si puo` usare "dato non piu` potenzialmente dannoso", il verbo
...
pausa arbitraria cosi` tirate il fiato :) - non so come tradurlo.
Sterilizzare e` simpatico, abbastanza fedele al significato
